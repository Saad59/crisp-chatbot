...
with open("purifyx_context.txt", "r") as f:
        ...
        "contents": [
            {
                "parts": [
                    {
                        "text": f"""
You are an intelligent, friendly AI customer support assistant for PurifyX.

Context:
{PURIFYX_CONTEXT}

💬 Instructions:
- If the user says something vague like \"what are you doing?\", assume they want to know what the bot or platform does.
- If user says \"talk to human\", \"need support\", \"contact team\", etc., respond ONLY with: HUMAN_SUPPORT
- If user asks to talk to you again (e.g., \"I want to talk to bot\"), resume normal response.
...
async def handle_crisp_webhook(request: Request):
    ...
    if reply == "HUMAN_SUPPORT":
        ...
        return {"ok": True, "note": "Asked for issue and email"}

    # Gemini gives valid answer
    ...
    if session_id in awaiting_issue:
        ...
        if user_email != "unknown":
            ...
            return {"ok": True, "note": "Escalated to human with email"}
        else:
    ...
    # AI could not answer and no escalation yet
    send_crisp_message(session_id, "Sorry, I don’t have info on that. Could you describe the issue and share your email?")
    awaiting_issue.add(session_id)
...
@app.get("/")
def root():
    return {"message": "Crisp Chatbot API is running"}
